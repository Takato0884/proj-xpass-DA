...
8. [Claude ゼロショット GIAA](#claude-ゼロショット-giaa)
9. [分析ツール](#分析ツール)
...
`src/methods/claude_giaa.py` は、学習済みモデルを使わず **Claude claude-opus-4-6** をゼロショット評価器として用いる GIAA ベースライン実装です。train/test split なしでジャンルフォルダの**全画像**を対象に推論し、Anthropic Batch API で一括送信・非同期取得します。

### 評価の仕組み

各画像に対して以下のシステム・ユーザープロンプトを送り、7クラスの評定分布（確率ベクトル）を JSON 形式で出力させます。
...
**ユーザープロンプト**（art の例）
> "Imagine approximately 13 ordinary people with no special training in art or photography are shown the art image below and asked to rate its aesthetic quality.
> …（7段階尺度の説明）…
> Predict the distribution of their ratings as a probability distribution over scores 1 through 7. The 7 probabilities must sum to 1.0.
> Respond only with a valid JSON array of exactly 7 floats …"

- ジャンルに応じてラベルを `art image` / `fashion image` / `landscape image` と切り替える
- 出力パース失敗時は一様分布（1/7 × 7）にフォールバック

### 処理フロー

```
ジャンルフォルダの全画像を base64 エンコード
    ↓
Batch API へ一括送信（1画像 = 1リクエスト）
    ↓
60秒ごとにポーリング（processing_status == "ended" まで）
    ↓
各結果の JSON 配列をパース → 確率分布に正規化
    ↓
reports/exp/claude/{genre}_results.json に保存
```
...
# 先頭10枚だけ試す（お試しモード）
python -m src.methods.claude_giaa --genre art --trial 10
```
...
| `--trial` | int | `0` | **お試しモード**: 先頭 N 件のみ評価（`0` = 全件） |

### 出力

`reports/exp/claude/{genre}_results.json` に以下の情報を保存します。
...
> **注意:** 実行には `ANTHROPIC_API_KEY` 環境変数（または `.env` ファイル）が必要です。Batch API の処理時間はリクエスト数によって数分〜数十分かかります。
