...
1. [環境構築](#環境構築)
2. [データ前処理](#データ前処理)
3. [データ分割](#データ分割)（[GIAAモード](#giaaモード--mode-giaa) / [PIAAモード](#piaaモード--mode-piaa)）
...
`--mode` で **piaa**（ユーザー単位CV）と **giaa**（画像単位CV）を切り替えます。

#### GIAAモード（`--mode giaa`）

全ユニーク画像を均等にn_folds個のグループに分割し、各foldでテスト画像を交代させます。各画像は必ずいずれか1つのfoldでテストになります。

```bash
# 全ジャンル（art, fashion, scenery）の5-fold 画像CV分割を生成
...
| fold | test | val | train |
|------|------|-----|-------|
| fold i | 全画像の約20%（fold i のグループ） | 残り80%からランダムに10% | 残り |

**出力（fold1の例）:**
- `data/split/v_giaa_fold1/art/train_images_GIAA.txt`
- `data/split/v_giaa_fold1/art/val_images_GIAA.txt`
- `data/split/v_giaa_fold1/art/test_images_GIAA.txt`

#### PIAAモード（`--mode piaa`）

全ユーザーを均等にn_folds個のグループに分割し、各foldでテストユーザーを交代させます。各ユーザーは必ずいずれか1つのfoldでテストユーザーになります。

```bash
# 全ジャンル（art, fashion, scenery）の5-fold ユーザーCV分割を生成（デフォルト）
...
- `data/split/v3_fold1/art/train_PIAA.txt` - PIAA学習用（形式: `user_id\tfilename`）
- `data/split/v3_fold1/art/val_PIAA.txt` - PIAA検証用（形式: `user_id\tfilename`）
- `data/split/v3_fold1/art/test_PIAA.txt` - PIAAテスト用（形式: `user_id\tfilename`）
...
| `--mode` | str | `piaa` | CVモード: `piaa`（ユーザー単位）/ `giaa`（画像単位） |
...
| `--val-frac-images-giaa` | float | `0.1` | **[piaa]** GIAA画像の検証データ割合（0〜1） |
| `--val-frac-users-giaa` | float | `0.1` | **[piaa]** GIAAユーザーの検証データ割合（0〜1） |
| `--n-train-piaa` | int | `100` | **[piaa]** PIAAのユーザーごとの学習画像数 |
| `--n-test-piaa` | int | `50` | **[piaa]** PIAAのユーザーごとのテスト画像数 |
| `--val-frac-giaa` | float | `0.1` | **[giaa]** 訓練画像から検証に使う割合（0〜1） |
| `--seed` | int | `42` | 乱数シード |
...
| `--wandb_project` | str | `XPASS` | wandbプロジェクト名 |
...
$$\text{Discrepancy}(F_1, F_2; x^t) = \mathbb{E}_{x^t}\left[\text{EMD}(F_1(G(x^t)),\ F_2(G(x^t)))\right]$$

各バッチで以下の3ステップを実行します：

1. **Step A — ソース学習**：G・F1・F2を両ヘッドのEMD損失平均で更新する  
   $\min_{G, F_1, F_2}\ \mathcal{L}_s$
2. **Step B — Discrepancy最大化（G固定）**：F1・F2がターゲットで食い違うよう更新する  
   $\min_{F_1, F_2}\ \mathcal{L}_s - \lambda \cdot \mathcal{L}_\text{adv}$
3. **Step C — Discrepancy最小化（F1/F2固定、n回繰り返し）**：Gがターゲット特徴をソースのサポートに引き込む  
   $\min_{G}\ \mathcal{L}_\text{adv}$

実装上の詳細：
- Gは `backbone（凍結）+ feat_proj`、F1は既存の `fc_aesthetic`、F2は独立して初期化した同形の線形層
- DiscrepancyはL1-EMD（累積和のL1距離）を使用。ソース損失はL2-EMDのまま
- OptimizerはG用（`optimizer_G`）とF1+F2用（`optimizer_F`）の2本立て（AdamW）
- Early stoppingの基準はソースVal EMD。ターゲットVal EMDは参考値としてwandbに記録
- 推論時はF1（`fc_aesthetic`）のみ使用。F2の重みは保存されない
...
# art → fashion
python -m src.train_GIAA --genre art --da_method MCD-fashion \
  --dataset_ver v_giaa --mcd_lambda 10.0 --mcd_n_steps 4

# art → scenery
python -m src.train_GIAA --genre art --da_method MCD-scenery \
  --dataset_ver v_giaa

# fashion → scenery
python -m src.train_GIAA --genre fashion --da_method MCD-scenery \
  --dataset_ver v_giaa
```
...
学習済みモデルを使わず、マルチモーダル LLM をゼロショット評価器として用いる GIAA / PIAA ベースライン実装です。Claude・GPT・Gemini の3モデルに対応しており、`--mode` で GIAA と PIAA を切り替えます。いずれも train/test split なしでジャンルフォルダの**全画像**（または全ペア）を対象に逐次推論し、100件ごとにチェックポイントを保存します。

### モデル別の設定

| モデル | モジュール | 使用モデル | 必要な環境変数 | 出力ディレクトリ |
|--------|-----------|-----------|--------------|----------------|
...
| Gemini | `src.methods.gemini` | `gemini-3-flash-preview` | `GEMINI_API_KEY` または `GOOGLE_API_KEY` | `reports/exp/gemini/` |

### 共通プロンプト設計

すべてのモデルで同一のシステム・ユーザープロンプトを使用します。
...
- ジャンルに応じてラベルを `art image` / `fashion image` / `landscape image` と切り替える
...
**ユーザープロンプト（PIAA）**：年齢・性別・学歴・国籍・ドメイン学習経験・興味・ビッグファイブ性格質問（Q1〜Q10）を含む個人プロファイルを埋め込み、1〜7 の整数スコアで回答させる

---

### GIAA モード

各画像について 7 クラスの評定分布（確率ベクトル）を JSON 形式で予測します。出力パース失敗時は一様分布（1/7 × 7）にフォールバックします。

#### コマンド例

```bash
# Claude: art ジャンルの全画像を評価
python -m src.methods.claude --mode giaa --genre art

# GPT
python -m src.methods.gpt --mode giaa --genre art

# Gemini（--resume で途中から再開可能）
...
`reports/exp/{model}/{genre}_results.json`（GPT・Gemini は `{genre}_results_sequential.json`）
...
- `pred_dist` はスコア 1〜7 に対応する予測確率分布（7要素、合計 1.0）

---

### PIAA モード

`ratings.csv` に基づき、ジャンル内の全 **（画像, ユーザー）ペア** を対象にスカラースコア（1〜7 の整数）を予測します。パース失敗時は 4（中央値）にフォールバックします。全モデルで `--resume` に対応しています。

#### コマンド例

```bash
# Claude: art ジャンルの全ペアを評価
python -m src.methods.claude --mode piaa --genre art

# GPT
python -m src.methods.gpt --mode piaa --genre art

# Gemini
python -m src.methods.gemini --mode piaa --genre art
...
| 引数 | 型 | デフォルト | 説明 |
|------|----|-----------|------|
| `--mode` | str | (必須) | 評価モード（`giaa` / `piaa`） |
| `--genre` | str | (必須) | 評価ジャンル（`art` / `fashion` / `scenery`） |
| `--trial` | int | `0` | 先頭 N 件の画像のみ評価（`0` = 全件） |
| `--resume` | flag | `False` | 既存の結果 JSON から処理済みをスキップして再開（GIAA は Gemini のみ対応） |

> **注意:** 実行には各モデルの API キーを環境変数（または `.env` ファイル）に設定する必要があります。
...
### 画像あたり評定者数（`data/maked/ratings.csv`）

| genre | 画像数 | 平均評定者数 | 標準偏差 | 最小 | 最大 |
|-------|-------:|------------:|---------:|-----:|-----:|
| art | 2,345 | 12.81 | 3.37 | 5 | 17 |
| fashion | 2,082 | 12.79 | 3.39 | 5 | 17 |
| scenery | 2,099 | 12.70 | 3.38 | 5 | 17 |
| **全体** | **6,526** | **12.77** | **3.38** | **5** | **17** |
